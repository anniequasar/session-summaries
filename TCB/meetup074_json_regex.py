# -*- coding: utf-8 -*-
"""meetup074_json_regex.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1El9hOgXGbhwkOnBpv0WrFnSEBZxfTt4B
...
from urllib.request import urlopen, urlretrieve  # standard library. Only import urlopen and urlretrieve function so don't need to prefix
...
print(f"Dumping Python int to json string {json_str=}")  # print() by default shows display formatted value. No difference between str and int
...
c = 3+4j
ps = repr(3+4j)  # repr() is similar to json.loads() for python object notation except json.loads only works on a few datatypes
print(f"{c=}, {ps=}, {repr(3+4j)=}")

b = eval(ps)  # eval() is similar to json.dumps() for python object notation except eval could evaluate full expressions, not just object literals. SECURITY RISK!!
...
for event in events:
    event['time_start'] = datetime.datetime.strptime(f"{event['startDateTime']} {event['startTimeZoneOffset']}", "%Y-%m-%dT%H:%M:%S %z")
    event['time_end'] = datetime.datetime.strptime(f"{event['endDateTime']} {event['endTimeZoneOffset']}", "%Y-%m-%dT%H:%M:%S %z")
for event in events[20:41]:
    print(f"Start {event['time_start']} from {event['startDateTime']} {event['startTimeZoneOffset']}, End {event['time_end']} from {event['endDateTime']} {event['endTimeZoneOffset']}")
...
for event in events:
    for field in event['customFields']:
        if field['label'] == 'Cost' and not free.search(field['value']):
            print(f"{event['location']}: {html.unescape(event['title'])}: {event['time_start']}: {event['categoryCalendar']}:", end=' ')
            print(field['value'])
            break
