...
plotly.io.write_html(fig, "demo1.html")
...
idx_start = series_sum.index[series_sum >= 100][0]
df_sum = pd.DataFrame(series_sum, columns=['current'])
df_sum['previous'] = df_sum['current'].shift(averaged_days)
...
df_plot['flt_previous'] = df_sum['current'].shift(averaged_days).astype(float)
df_plot['doubling days'] = averaged_days / np.log2(df_plot['flt_current'] / df_plot['flt_previous'])
