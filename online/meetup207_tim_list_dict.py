r"""MeetUp 207 - Beginners' Python and Machine Learning - 07 Aug 2024 - Getting Started 2
...
Links:
- Colab:   https://colab.research.google.com/drive/1A9rvk_mLfck42UUPvbnZHmbPW9QEKuVA
- Youtube: https://youtu.be/UVBIrp0mRac
- Meetup:  https://www.meetup.com/beginners-python-machine-learning/events/302440683/
...
It is the second in the series targeted at absolute beginners after meetup 191.
...
print(f"tpl + tpl returns {tpl + tpl}")
print(f"3 * tpl returns {3 * tpl}")
print(f'tpl + lst returns TypeError: can only concatenate tuple (not "list") to tuple')
...
# Unpack operator works for tuples. It works for lists as well
print("Using unpack operator", *tpl)
...
i = 30
print("\nUsing maths to calculate index and select name of number from tuple")
...
print("\nUsing dict index to select name of number")
print(f"{i=}, {tens[i]=}")
...
print("\nExample using str keys to convert 'python' to phonetic alphabet")
for letter in word:
...
# Unpacking dicts
# Just like we could unpack lists using * operator, we can unpack dicts using ** operator
# This is how we would normally call a function using keyword arguments
print("\nUsing named arguments in print function")
print(3, 2, 1, sep='-+-', end='\n================\n')

# Unpacking in function call gives us argument keyword and value
# Advantages:
#  1. we can set up parameters elsewhere in program  
#  2. in a conditional where the result determines whether we provide a particular argument we don't need to have two branches of code calling same function
#  3. if we want to pass kwargs to one function which will pass them on to another we can use unpacking and packing operators. Outer function doesn't need to know all parameters of inner function
kwargs = {'end': '\n__________________\n', 'sep':'|-|'}
print("\nUnpacking kwargs in print function")
print(3, 2, 1, **kwargs)