...
tens = {20: "twenty", 30: "thirty", 40: "forty", 50: "fifty", 60: "sixty",
        70: "seventy", 80: "eighty", 90: "ninety"}  # 7
powers = {1000000000000: "trillion", 1000000000: "billion", 1000000: "million",
...
# DONE rename num to number
numerals = 345
print(f"number_name({numerals})={number_name(numerals)}")  # 6
# print(f"{number_name(num)=}")  # 8
assert number_name(345) == "three hundred and forty-five"
numerals = 456  # 10
print(f"number_name({numerals})={number_name(numerals)}")  # 6
numerals = 12345040302
print(f"number_name({numerals})={number_name(numerals)}")
...
c = 3+4j
print(f"{c:7.2f}°C is equivalent to {fahrenheit(abs(c)):7.2f}°F")  # 14
...
class Invoice(Document):
    def __init__(self, aud_amount, days_terms=30, date_document=None):  # 16
        ...
        """
        super().__init__("Invoice", "payment due", aud_amount, days_terms, date_document)  # 15
    ...
    def adjustment_note(self):
        ...
        for item in self.lst_items:
            ...
            adj_note.add_item(Item(item.description, -item.quantity, item.aud_unit_sell))  # 17
        return adj_note
...
invoices = [iv3, iv2, iv1]  # 13

for iv in invoices:
    if hasattr(iv, "original_invoice"):
