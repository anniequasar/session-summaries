#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""meetup079_classes.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1gYiHBy0YfaDPteCYAFgdOp074ejH27vG
...
print(f"Attributes of class Complex")
...
iv = Invoice(350)
print(f"Solution 1: Invoice for ${iv.aud_amount:,.2f} dated {iv.date_document.strftime('%d %B %Y')} is due {iv.date_due().strftime('%d %B %Y')}")
...
def str_invoice(self):
    return f"Invoice for ${self.aud_amount:,.2f} dated {self.date_document.strftime('%d %B %Y')} is due {self.date_due().strftime('%d %B %Y')}"    
...
Invoice.__str__ = lambda self: f"Invoice for amount ${self.aud_amount:,.2f} dated {self.date_document.strftime('%d %b %Y')} is due {self.date_due().strftime('%d %b %Y')}" 
