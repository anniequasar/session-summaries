# -*- coding: utf-8 -*-
"""meetup070_Tim_rss_web_scraping

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1RksbsUKRf5zik1lhGmLG5jsEo0amOOnh
...
print(f"p2.status {p2.status}, http.HTTPStatus.NOT_MODIFIED {http.HTTPStatus.NOT_MODIFIED}, http.HTTPStatus.NOT_MODIFIED == p2.status {http.HTTPStatus.NOT_MODIFIED == p2.status}")
...
# Solution 3:
print("\n\nSOLUTION 3\n")
print(f"'soup.exchangeRate' or 'soup.find('exchangeRate') find the first 'exchangeRate' Tag in the soup")
print(soup.find('exchangeRate'))

# .text extracts the text from Tag and all descendents
# .string extracts the text from Tag only if it is unambiguous what should be returned eg no subtags, or exactly one direct subtag and it has a string
# .strings extracts list of .string from all descendents
print(f"'soup.exchangeRate.text' extracts the text from all tags in the first 'exchangeRate' Tag in the soup\n{soup.exchangeRate.text}\n")
...
print(f"'soup.targetCurrency.text' extracts the text from the first 'targetCurrency' Tag in the soup\n{soup.targetCurrency.text}\n")

print(f"'soup.targetCurrency.string' extracts the text from the first 'targetCurrency' Tag in the soup only if unambiguous\n{soup.targetCurrency.string}\n")
...
anchor_bne = accc.find(attrs={'id': 'petrol-prices-in-brisbane'})  # this form useful to search on attributes such as name or data-foo that can't be used in kwargs
print("\n\nSOLUTION 6\n")
print(anchor_bne)
...
# Task 10: From the RBA page showing rates over the last three days, find currencies where rate has both increased and decreased over that time period
...
for tr in trs:
    tds = tr.find_all('td')
    print(f"{tr['id']:7} {tr.th.string:26}, r1 {align_point(tds[0].string)}, r2 {align_point(tds[1].string)}, r3 {align_point(tds[2].string)}")
