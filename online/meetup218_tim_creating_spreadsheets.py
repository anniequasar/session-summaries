#!/usr/bin/env python3
r"""MeetUp 218 - Beginners' Python and Machine Learning - 16 Jul 2025 - Creating spreadsheets from Python
...
Links:
- Colab:   <https://colab.research.google.com/drive/12NRCm1MJbjMCsgkP3ziRfF_WRodtuoIB>
- Youtube: <https://youtu.be/Z7dAntvUXuc/>
- Meetup:  <https://www.meetup.com/beginners-python-machine-learning/events/308804401/>
...
#     bpaml\Scripts\Activate.bat  # Windows
...
# Run script
#     python online/meetup218_tim_creating_spreadsheets.py  # Linux or MacOS
#     python online\meetup218_tim_creating_spreadsheets.py  # Windows
...
lst_scripts = list(online_directory.glob("*_tim_*.py")) + list(online_directory.glob("*_tim_*.md")) + list(online_directory.glob("*_tim_*.ipynb"))
# Sort the list using sort method
lst_scripts.sort()
print("Names of python scripts in online directory of bpaml-sessions repo")
...
import json  # javascript object notation standard library for reading jupyter notebooks
...
for script_path in lst_scripts:
    ...
    with open(script_path, "r", encoding="UTF-8") as f:
        if script_path.suffix in [".py", ".md"]:
            lines = f.readlines()
        else:  # .ipynb is a json file
            dct_notebook = json.load(f)
            lines = dct_notebook['cells'][0]["source"]
        for line in lines:
...
print("\nSolution 5: Finding Youtube links defined in a function so we can use it later")
def parse_py_lines(path_script, lst_title):
    ...
    # search for title followed by non-greedy optional characters .*? then : then optional spaces \s* then URL (any chars except <> or []) surrounded optionally by <> or []
    dct_ptn_title = {k:re.compile(re.escape(k) + r".*?:\s*\[?<?(http[^>^\]]*)>?\]?", re.IGNORECASE) for k in lst_title}
    with open(path_script, "r", encoding="UTF-8") as f:
        if path_script.suffix in [".py", ".md"]:
            lines = f.readlines()
        else:  # ipynb is json
            dct_notebook = json.load(f)
            lines = dct_notebook['cells'][0]["source"]
        for line in lines:
            if "docstring" not in first:
                # := is the walrus operator. Assigns to m and checks for Truthy
...
with xlsxwriter.Workbook("bpaml-links-6.xlsx") as workbook:
    ...
    for i, script in enumerate(lst_scripts):
        ...
        for j, title in enumerate(["Youtube", "Meetup", "Colab"]):
            url, str_last = url_last(dct, [title])
            if url:
                worksheet.write_url(i + 1, j + 2, url, string=str_last)
        # Source URL could be labelled "Github" or "Source" but not both
        url, str_last = url_last(dct, ["Source", "Github"])
        if url:
            # For many URLs I have been lazy and just given URL to folder
            if str_last == "online":
                str_last = script.name
                url = f"https://github.com/timcu/bpaml-sessions/blob/master/online/{str_last}"
            print(str_last, url)
