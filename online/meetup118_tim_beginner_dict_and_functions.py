#!/usr/bin/env python3
"""MeetUp 118 - Beginners' Python and Machine Learning - 05 Oct 2021 - Getting Started 2

- Youtube: https://youtu.be/pk-SBS70A-M
- Colab:   https://colab.research.google.com/drive/1q477bqdCEeK3pGBRdMql6oxJPzX3q9JA
- Meetup:  https://www.meetup.com/Beginners-Python-Machine-Learning/events/281083136/
- Github:  https://github.com/timcu/bpaml-sessions/tree/master/online

Learning objectives:

Learn how to use a Python `dict` (short for dictionary). A Python `dict` is like
a `list` on steroids. Not only can you store values of any data type in a `dict` (just like a `list`) but you can specify what index key to retrieve the data later. This can be used to look up data and save relationships of data.

You will also learn how to define functions so you can reuse code without retyping.

Follows on from Meetup 114 - Getting Started 1
...
"""
import datetime  # gives us access to the standard library datetime
...
my_tuple  # will be displayed in Jupyter Notebooks but not in Python
...
print(f"my_str   : {my_str!r}")  # !r is equivalent to repr(). displays internal representation. useful to distinguish data type such as str in this case
...
# d2 = {[1,4]: "bottom-left"}
print(f"Keys can't be mutable or unhashable such as list or dict")

# Add new keys and values. Keys have to be unique or they will overwrite previous value
d['d'] = 'delta'
d['e'] = 'epsilon'
print(f"dict after assigning values to new keys\n{d}")

# Merge 
d.update({'e': 'echo', 'f': 'foxtrot'})
print(f"Updated (merged) dict\n{d}")
...
for i in [27, 52, 5, 12, 30, 100, 112, 300, 999, 0]:
...
# Uses `import datetime` at top of script
def show_now():
