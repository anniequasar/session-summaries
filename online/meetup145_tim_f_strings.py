...
# 
# 
# It is the third in the series targeted at absolute beginners after meetup 141.
# 
# # Using Datalore / Jupyter Notebooks / IPython
# 
# - type into a cell
...
# To convert str to date strip the last three characters and use strptime
# To convert date to str use strftime
d = datetime.strptime('2020/10/09 00:00:00+00'[:-3], '%Y/%m/%d %H:%M:%S')
...
for r in lst_records:
    d = datetime.strptime(r['LAST_INSPECTION_DATE'][:-3], '%Y/%m/%d %H:%M:%S')
    print(f"{r['DESCRIPTION']:35} {float(r['LATITUDE']):10.3f} {float(r['LONGITUDE']):10.3f} "
